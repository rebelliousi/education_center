...
const Activities = () => {
  ...
  // Tüm aktiviteleri döngüsel yap (sonsuz kaydırma için)
  const extendedActivities = [...activities, ...activities, ...activities]
  
  // Görünür aktiviteleri hesapla
  const visibleActivities = extendedActivities.slice(
    currentIndex,
  ...
  const paginate = (newDirection: number) => {
    ...
    setCurrentIndex((prevIndex) => {
      ...
      if (newIndex >= activities.length) return 0
      // Eğer başa geldiyse sona git
      if (newIndex < 0) return activities.length - 1
  ...
  }, [isAutoPlaying, currentIndex, activities.length])
  ...
  // Slider container'ına hover olayları ekle
  const handleMouseEnter = () => setIsAutoPlaying(false)
  const handleMouseLeave = () => setIsAutoPlaying(true)

  // Doğru indicator index'ini hesapla
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 lg:h-72 items-stretch">
                      ...
                      <div className="p-4 sm:p-5 lg:p-6 flex flex-col justify-between">
                        <div>
                          ...
                          <p className="text-gray-600 text-xs sm:text-sm mb-3 lg:mb-4 line-clamp-2 lg:line-clamp-3">
                        ...
                        <button
                          ...
                          onClick={() => {
                            // Orijinal aktiviteyi bul
                            const originalIndex = (currentIndex + index) % activities.length
                            setActiveActivity(activities[originalIndex]);
        ...
        <div className="flex justify-center gap-1.5 sm:gap-2 mt-6 sm:mt-8 lg:mt-12">
          {activities.map((_, index) => (
            <button
              key={index}
              onClick={() => {
                setDirection(index > getIndicatorIndex() ? 1 : -1)
