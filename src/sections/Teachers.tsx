    ...
    <section id="teachers" className="py-12 sm:py-16 lg:py-24 bg-gradient-to-br from-blue-50 via-white to-blue-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <motion.div
          ...
          viewport={{ once: true }}
          className="bg-gradient-to-r from-blue-600 to-blue-700 rounded-2xl sm:rounded-3xl p-6 sm:p-8 lg:p-12 mb-10 sm:mb-16 lg:mb-20 text-white shadow-xl"
        >
            ...
            {top3Teachers.map((teacher, index) => {
              ...
              return (
                <motion.div
                  ...
                  className="bg-white/15 backdrop-blur-md rounded-xl sm:rounded-2xl p-4 sm:p-6 lg:p-8 text-center border border-white/30 hover:bg-white/20 transition-all duration-300 group cursor-pointer"
                >
                  <div className="relative mb-4 sm:mb-6">
                    <img
                      src={getTeacherImage(teacher)}
                      alt={getTranslated(teacher, "name")}
                      className="w-20 h-20 sm:w-24 sm:h-24 rounded-full mx-auto object-cover border-4 border-white/40 group-hover:scale-110 transition-transform duration-300"
                    />
                    <div className="absolute -top-2 -right-2 sm:-top-3 sm:-right-3 bg-gradient-to-r from-yellow-300 to-yellow-400 text-gray-900 rounded-full w-8 h-8 sm:w-10 sm:h-10 flex items-center justify-center font-bold text-base sm:text-lg shadow-lg">
                      {index + 1}
                    </div>
                  </div>
                  <h4 className="font-bold text-base sm:text-lg lg:text-xl mb-1 sm:mb-2">{getTranslated(teacher, "name")}</h4>
                  <p className="text-blue-100 text-xs sm:text-sm mb-3 sm:mb-4 line-clamp-1">{getTranslated(teacher, "specialization")}</p>
                  <div className="flex items-center justify-center space-x-1.5 sm:space-x-2 bg-white/10 rounded-full py-1.5 px-3 sm:py-2 sm:px-4 w-fit mx-auto">
                    <Heart className="h-4 w-4 sm:h-5 sm:w-5 text-red-300" />
                    <span className="font-bold text-base sm:text-lg">{currentLikes.toLocaleString()}</span>
                  </div>
                  <div className="mt-3 sm:mt-4">
                    {getAchievements(teacher).length > 0 && (
                      <ul className="text-[10px] sm:text-xs text-blue-100 space-y-0.5">
          ...
          {top8Teachers.map((teacher, index) => {
            ...
            return (
              <motion.div
                ...
                viewport={{ once: true }}
                whileHover={{ y: -12 }}
                className="bg-white/50 backdrop-blur-sm rounded-xl sm:rounded-2xl overflow-hidden border border-blue-200/50 hover:border-blue-300 transition-all duration-300 group shadow-lg hover:shadow-xl"
              >
                {/* Teacher Image */}
                <div className="relative h-48 sm:h-56 overflow-hidden bg-gradient-to-br from-blue-100 to-blue-200">
                  <img
                    src={getTeacherImage(teacher)}
                    alt={getTranslated(teacher, "name")}
                  ...
                  {teacher.featured && (
                    <motion.div 
                      initial={{ scale: 0 }}
                      animate={{ scale: 1 }}
                      className="absolute top-2 left-2 sm:top-4 sm:left-4 bg-gradient-to-r from-yellow-300 to-yellow-400 text-gray-900 px-2 py-1 sm:px-3 sm:py-1.5 rounded-full text-[10px] sm:text-xs font-bold shadow-lg"
                    >
                      ⭐ {t("teachers.featured")}
                    </motion.div>
                  )}
                  ...
                  <motion.button
                    ...
                    className={`absolute top-2 right-2 sm:top-4 sm:right-4 w-10 h-10 sm:w-14 sm:h-14 rounded-full flex items-center justify-center transition-all duration-300 shadow-lg ${
                      isLiked 
                  ...
                  <div className="absolute bottom-2 left-2 right-2 sm:bottom-4 sm:left-4 sm:right-4">
                    <div className="bg-white/20 backdrop-blur-md rounded-lg sm:rounded-xl p-2 sm:p-3 border border-white/30">
                      <div className="flex items-center justify-between text-white text-xs sm:text-sm font-semibold">
                        ...
                        <div className="flex items-center space-x-1 sm:space-x-2">
                          <Heart className="h-3 w-3 sm:h-4 sm:w-4 text-red-300" />
                ...
                <div className="p-4 sm:p-6">
                  <h3 className="text-base sm:text-lg font-bold text-gray-900 mb-1 sm:mb-2 group-hover:text-blue-600 transition-colors line-clamp-1">
                  ...
                  <p className="text-gray-700 text-xs sm:text-sm mt-1.5 sm:mt-2 line-clamp-2">{getTranslated(teacher, "bio")}</p>
                  {getAchievements(teacher).length > 0 && (
                    <ul className="text-[10px] sm:text-xs text-blue-600 mt-1.5 sm:mt-2 space-y-0.5">
                      {getAchievements(teacher).slice(0, 2).map((ach: string, idx: number) => (
                        <li key={idx} className="line-clamp-1">🏅 {ach}</li>
                      ))}
                    </ul>
